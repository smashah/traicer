...
function App() {
  ...
  const refresh = async () => {
    try {
      setStatus(await invoke<DaemonStatus>("daemon_health"));
    } catch {
  ...
  const configure = async (event: React.FormEvent) => {
    ...
    setError(undefined);
    try {
      if (!marketplaceCredential || !storageSecret) throw new Error("Marketplace and storage credentials are required");
  ...
  return <main style={{ background: "#f1f5f9", color: "#0f172a", fontFamily: "Inter, system-ui, sans-serif", minHeight: "100vh", padding: 20 }}>
    ...
    {status.running ? <section style={{ background: "white", borderRadius: 14, display: "grid", gap: 10, marginBottom: 16, padding: 16 }}><h2 style={{ fontSize: 16, margin: 0 }}>Local trace lifecycle</h2><p style={{ color: "#64748b", fontSize: 12, margin: 0 }}>Deletion removes seller storage ciphertext, submits a signed marketplace tombstone, and then erases local trace references.</p>{traces.length === 0 ? <p style={{ color: "#64748b", fontSize: 13, margin: 0 }}>No local trace metadata is available.</p> : traces.map((trace) => <div key={trace.traceId} style={{ alignItems: "center", border: "1px solid #cbd5e1", borderRadius: 10, display: "flex", gap: 10, padding: 12 }}><div style={{ flex: 1 }}><strong style={{ fontFamily: "monospace", fontSize: 12 }}>{trace.traceId}</strong><p style={{ color: "#475569", fontSize: 11, margin: "4px 0 0" }}>{trace.state} · {new Date(trace.capturedAt).toLocaleString()}</p></div>{trace.state === "committed" ? <button disabled={busy} onClick={() => void deleteTrace(trace.traceId)} style={{ ...buttonStyle, background: "#b42318" }}>Delete and tombstone</button> : null}</div>)}</section> : null}

    <form onSubmit={configure} style={{ background: "white", borderRadius: 14, display: "grid", gap: 10, padding: 16 }}><h2 style={{ fontSize: 16, margin: 0 }}>Configure or rotate device</h2><p style={{ color: "#475569", fontSize: 12, lineHeight: 1.5, margin: 0 }}>Secrets are transferred once to the daemon over stdin and stored in the operating-system credential vault. They are never written to logs, process arguments, or marketplace payloads.</p><select style={inputStyle} value={provider} onChange={(event) => { const next = event.target.value as Provider; setProvider(next); setClient(next === "anthropic" ? "claude-code" : "codex"); }}><option value="openai">OpenAI-compatible · Codex/OpenCode</option><option value="anthropic">Anthropic Messages · Claude Code</option></select><input style={inputStyle} value={client} onChange={(event) => setClient(event.target.value)} placeholder="Client label" required /><input style={inputStyle} value={marketplaceApi} onChange={(event) => setMarketplaceApi(event.target.value)} placeholder="Marketplace API" type="url" required /><input style={inputStyle} value={marketplaceCredential} onChange={(event) => setMarketplaceCredential(event.target.value)} placeholder="Marketplace scoped credential" type="password" required /><input style={inputStyle} value={endpoint} onChange={(event) => setEndpoint(event.target.value)} placeholder="S3-compatible HTTPS endpoint" type="url" required /><input style={inputStyle} value={bucket} onChange={(event) => setBucket(event.target.value)} placeholder="Seller bucket" required /><div style={{ display: "grid", gap: 8, gridTemplateColumns: "1fr 1fr" }}><input style={inputStyle} value={prefix} onChange={(event) => setPrefix(event.target.value)} placeholder="Prefix" required /><input style={inputStyle} value={region} onChange={(event) => setRegion(event.target.value)} placeholder="Signing region" required /></div><input style={inputStyle} value={storageAccessKeyId} onChange={(event) => setStorageAccessKeyId(event.target.value)} placeholder="Storage access key ID" type="password" required /><input style={inputStyle} value={storageSecret} onChange={(event) => setStorageSecret(event.target.value)} placeholder="Storage secret access key" type="password" required /><button disabled={busy} style={buttonStyle} type="submit">{busy ? "Applying secure configuration…" : "Authorise device and start"}</button></form>
