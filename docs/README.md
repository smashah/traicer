...
- `apps/cli` — the published `@traice-market/traicer` package: file-based initializer, secret encryption, and daemon launcher.
...
- `packages/` — shared workspace packages: `domain`, `capture-core`, `privacy-pipeline`, `crypto`, `storage-s3`, `state-sqlite`, `marketplace-client`, `api-contract`, and `effect-runtime`.
